...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="fw-bold"><i class="bi bi-pencil text-primary me-2"></i> تعديل فاتورة الورشة #{{ $invoice->invoice_number }}</h3>
        <a href="{{ route('workshops.show', $invoice->workshop_id) }}" class="btn-glass-secondary">عودة</a>
    </div>
    ...
    <form action="{{ route('workshops.invoices.update', $invoice->id) }}" method="POST" id="invoiceForm">
        @csrf
        @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="workshop_id" class="form-select select2" required>
                            ...
                            @foreach($workshops as $workshop)
                                <option value="{{ $workshop->id }}" {{ $invoice->workshop_id == $workshop->id ? 'selected' : '' }}>
                                    {{ $workshop->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="number" step="0.01" name="paid_amount" id="paidAmount" class="form-control form-control-lg text-center fw-bold" value="{{ $invoice->paid_amount }}" oninput="calculateTotals()">
...
<script>
    ...
    // Existing Items
    const existingMaterials = @json($invoice->items->where('item_type', 'raw_material')->values());
    const existingProducts = @json($invoice->items->where('item_type', 'product')->values());

    let matIndex = 0;
    let prodIndex = 0;

    function addMaterialRow(itemId = '', qtyVal = '', priceVal = '') {
        let options = '<option value="">اختر خامة</option>';
        rawMaterials.forEach(m => {
            const selected = (m.id == itemId) ? 'selected' : '';
            options += `<option value="${m.id}" data-price="0" ${selected}>${m.name}</option>`;
        });
        ...
        const row = `
            <tr id="matRow${matIndex}">
                ...
                <td>
                    <input type="number" step="0.01" name="materials[${matIndex}][quantity]" class="form-control form-control-sm text-center mat-qty" value="${qtyVal}" required oninput="calculateTotals()">
                </td>
                <td>
                    <input type="number" step="0.01" name="materials[${matIndex}][price]" class="form-control form-control-sm text-center mat-price" value="${priceVal}" required oninput="calculateTotals()">
                </td>
    ...
    function addProductRow(itemId = '', qtyVal = '', priceVal = '') {
        let options = '<option value="">اختر منتج</option>';
        products.forEach(p => {
            const selected = (p.id == itemId) ? 'selected' : '';
            options += `<option value="${p.id}" data-cages="${p.cages_per_carton}" ${selected}>${p.name}</option>`;
        });
        ...
        const row = `
            <tr id="prodRow${prodIndex}">
                ...
                <td>
                    <input type="number" step="1" name="products[${prodIndex}][quantity]" class="form-control form-control-sm text-center prod-qty" value="${qtyVal}" required oninput="updateCartonInfo(this, ${prodIndex}); calculateTotals();">
                </td>
                <td>
                    <input type="number" step="0.01" name="products[${prodIndex}][price]" class="form-control form-control-sm text-center prod-price" value="${priceVal}" required oninput="calculateTotals()">
                </td>
    ...
    // Load existing items or add initial empty row
    if (existingMaterials.length > 0) {
        existingMaterials.forEach(item => {
            addMaterialRow(item.item_id, item.quantity, item.unit_price);
        });
    } else {
        addMaterialRow();
    }
    ...
    if (existingProducts.length > 0) {
        existingProducts.forEach(item => {
            addProductRow(item.item_id, item.quantity, item.unit_price);
        });
    } else {
        addProductRow();
    }
