...
@php
    $title = 'كشف حساب ورشة مفصل';
@endphp

@section('title', $title . ' - ' . $workshop->name)
@section('customer_phone', $workshop->phone ?? '')
...
<div class="row mb-4">
    ...
    <div class="col-6">
        <div class="border border-2 border-dark p-3 h-100 rounded-3" style="background-color: #f8f9fa;">
            <h5 class="fw-bold border-bottom border-dark pb-2 mb-3">بيانات الورشة</h5>
            <p class="mb-2 fw-bold fs-4 text-primary">{{ $workshop->name }}</p>
            <p class="mb-2 fs-5"><i class="bi bi-telephone me-2"></i> {{ $workshop->phone ?? 'غير مسجل' }}</p>
            <p class="mb-0 fs-5"><i class="bi bi-geo-alt me-2"></i> {{ $workshop->address ?? 'غير مسجل' }}</p>
        </div>
    </div>
</div>

<div class="mb-4">
    <h5 class="fw-bold mb-3">تفاصيل الحركات (منصرف / وارد / تسويات)</h5>
    <table class="table table-bordered border-dark table-striped mb-0">
        <thead class="text-center table-light border-dark">
            <tr>
                ...
                <th style="width: 120px">دائن (لهم)</th>
                <th style="width: 120px">مدين (لنا)</th>
            </tr>
        </thead>
        <tbody class="fw-bold">
...
<div class="row mt-4">
    <div class="col-6">
        <div class="border border-dark p-3 mt-auto rounded-3 bg-light">
            ...
            <p class="mb-0">هذا الكشف يمثل ملخصاً لجميع العمليات المالية وفواتير الخامات والمنتجات المتعلقة بالورشة حتى تاريخ طباعته.</p>
        </div>
    </div>
    <div class="col-6">
        <table class="table table-bordered border-dark text-center fw-bold mb-0">
            <tr>
                ...
                <td class="fs-4 bg-light border-dark {{ $netBalance >= 0 ? 'text-danger' : 'text-success' }}">
                    @if($netBalance > 0)
                        مديون لنا بـ : {{ number_format(abs($netBalance), 2) }} ج.م
                    @elseif($netBalance < 0)
                        له عندنا بـ : {{ number_format(abs($netBalance), 2) }} ج.م
                    @else
                        مسفر (0.00 ج.م)
                    @endif
                </td>
