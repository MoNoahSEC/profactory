...
@section('customer_phone', $supplier->phone ?? '')
...
<div class="row mb-4">
    ...
    <div class="col-6">
        <div class="border border-2 border-dark p-3 h-100 rounded-3" style="background-color: #f8f9fa;">
            <h5 class="fw-bold border-bottom border-dark pb-2 mb-3">بيانات المورد</h5>
            <p class="mb-2 fw-bold fs-4 text-primary">{{ $supplier->name }}</p>
            @if($supplier->company_name)
                <p class="mb-2 fs-5"><i class="bi bi-building me-2"></i> {{ $supplier->company_name }}</p>
            @endif
            <p class="mb-2 fs-5"><i class="bi bi-telephone me-2"></i> {{ $supplier->phone ?? 'غير مسجل' }}</p>
            <p class="mb-0 fs-5"><i class="bi bi-geo-alt me-2"></i> {{ $supplier->address ?? 'غير مسجل' }}</p>
        </div>
    </div>
</div>

<div class="mb-4">
    <h5 class="fw-bold mb-3">تفاصيل الحركات (مشتريات وسدادات)</h5>
    <table class="table table-bordered border-dark table-striped mb-0">
        <thead class="text-center table-light border-dark">
            <tr>
                <th style="width: 50px">م</th>
                <th style="width: 120px">التاريخ</th>
                <th>البيان (فاتورة / سداد / تسوية)</th>
                <th style="width: 130px">لنا (دفعات)</th>
                <th style="width: 130px">لهم (مشتريات)</th>
            </tr>
        </thead>
        <tbody class="fw-bold">
...
<div class="row mt-4">
    <div class="col-6">
        <div class="border border-dark p-3 mt-auto rounded-3 bg-light">
            ...
            <p class="mb-0">هذا الكشف يمثل ملخصاً لجميع العمليات المالية ومشتريات الخامات المسجلة على النظام حتى تاريخ طباعته.</p>
        </div>
    </div>
    <div class="col-6">
        <table class="table table-bordered border-dark text-center fw-bold mb-0">
            <tr>
                <th class="w-50 fs-5 bg-light border-dark">الصافي النهائي للمديونية</th>
                <td class="fs-4 bg-light border-dark {{ $netBalance >= 0 ? 'text-danger' : 'text-success' }}">
                    @if($netBalance >= 0)
                        للمورد: {{ number_format(abs($netBalance), 2) }} ج.م
                    @else
                        لنا: {{ number_format(abs($netBalance), 2) }} ج.م
                    @endif
                </td>
            </tr>
        </table>
